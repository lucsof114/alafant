  ...
  if (transactionError && typeof transactionError !== 'string') {
    ...
    if ('InsufficientFundsForRent' in error) {
      return {
        ...
        code: 1,
        programIds: [splToken.TOKEN_PROGRAM_ID.toBase58()]
      };
    } else {
...
async function transactionSenderAndConfirmationWaiter({
  ...
  try {
    ...
    if (e instanceof web3_js.TransactionExpiredBlockheightExceededError) ; else {
      // invalid state from web3.js
      throw e;
    }
  }
  // in case rpc is not synced yet, we add some retries
  const response = promiseRetry__default["default"](async retry => {
...
const handleSendTransaction = async ({
  ...
  if (!skipPreflight) {
    ...
    } = await connection.simulateTransaction('message' in signedTransaction ? signedTransaction : new web3_js.VersionedTransaction(signedTransaction.compileMessage()), {
...
const deserializeMint = (data, address) => {
  if (data.length !== splToken.MintLayout.span) {
    // This is causing problem with mainnet token
    // throw new Error('Not a valid Mint')
    return;
  }
  const mintInfo = splToken.MintLayout.decode(data);
  ...
  mintInfo.supply = splToken.u64.fromBuffer(mintInfo.supply);
...
async function getWSolATA(connection, publicKey) {
  const wSolAddress = await splToken.Token.getAssociatedTokenAddress(splToken.ASSOCIATED_TOKEN_PROGRAM_ID, splToken.TOKEN_PROGRAM_ID, splToken.NATIVE_MINT, publicKey, true);
...
async function createAndTransferWSOLTransaction({
  ...
  if (!wSolAccount.accountInfo) {
    // Create ATA account
    instructions.push(splToken.Token.createAssociatedTokenAccountInstruction(splToken.ASSOCIATED_TOKEN_PROGRAM_ID, splToken.TOKEN_PROGRAM_ID, splToken.NATIVE_MINT, wSolAccount.address, publicKey, publicKey));
  }
  ...
  splToken.Token.createSyncNativeInstruction(splToken.TOKEN_PROGRAM_ID, wSolAccount.address));
...
async function createUnwrapSolTransaction({
  ...
  // Close account
  instructions.push(splToken.Token.createCloseAccountInstruction(splToken.TOKEN_PROGRAM_ID, wSolAccount.address, publicKey, publicKey, []));
  const transaction = new web3_js.Transaction({
    feePayer: publicKey,
    blockhash,
    lastValidBlockHeight
...
const TOKEN_METADATA_PROGRAM_ID = /*#__PURE__*/new web3_js.PublicKey('metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s');
// This is not an official program but a community deployement
const TOKEN_2022_METADATA_PROGRAM_ID = /*#__PURE__*/new web3_js.PublicKey('META4s4fSmpkTbZoUsgC1oBnWB31vQcmnN8giPw51Zu');
function findMetadataAddress(mint, tokenProgramId) {
  const metadataProgramId = tokenProgramId.equals(splToken.TOKEN_PROGRAM_ID) ? TOKEN_METADATA_PROGRAM_ID : TOKEN_2022_METADATA_PROGRAM_ID;
  return web3_js.PublicKey.findProgramAddressSync([Buffer.from('metadata'), metadataProgramId.toBuffer(), mint.toBuffer()], metadataProgramId)[0];
}
async function fetchMintInfos(connection, mints) {
  const results = (await anchor.utils.rpc.getMultipleAccounts(connection, mints)).reduce((acc, m, idx) => {
    if (m) {
      acc.push([mints[idx].toString(), {
        ...splToken.MintLayout.decode(m.account.data),
...
async function fetchTokenMetadatas(connection, tokens) {
  ...
  const tokenMetaAccounts = (await anchor.utils.rpc.getMultipleAccounts(connection, metadataAddresses)).reduce((acc, keyedAccount) => {
    if (keyedAccount) {
      const metadata = mplTokenMetadata.Metadata.deserialize(keyedAccount.account.data)[0];
...
exports.PROGRAM_FAILED_TO_COMPLETE_ERROR = PROGRAM_FAILED_TO_COMPLETE_ERROR;
exports.TransactionError = TransactionError;
exports.UNKNOWN_ERROR = UNKNOWN_ERROR;
exports.createAndTransferWSOLTransaction = createAndTransferWSOLTransaction;
...
exports.fetchMintInfos = fetchMintInfos;
exports.fetchTokenMetadatas = fetchTokenMetadatas;
exports.findMetadataAddress = findMetadataAddress;
exports.getSignature = getSignature;
exports.handleSendTransaction = handleSendTransaction;
exports.parseErrorForTransaction = parseErrorForTransaction;
exports.parseErrorWithIdl = parseErrorWithIdl;
...
//# sourceMappingURL=optimist.cjs.development.js.map
